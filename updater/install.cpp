...
static int do_SetMetadataRecursive(const char* filename, const struct stat* statptr, int fileflags __unused,
                                   struct FTW* pfwt __unused) {
...
Value* Tune2FsFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv __unused) {
